...
src_install() {
	...
	do
		...
		# Force use of lib32 path instead of lib for 32bit
		if [ "${ABI}" = "x86" ] ; then
			libdir="lib32"
			LIBDIR_x86="$libdir"
		else
		...
		dosym libocci$(get_libname 12.1) \
			"${oracle_home}"/$libdir/libocci$(get_libname)
		[[ -e libclntsh$(get_libname) ]] ||
		dosym libclntsh$(get_libname 12.1) \
			"${oracle_home}"/$libdir/libclntsh$(get_libname)

		# java archives
		insinto "${oracle_home}"/$libdir
		doins *.jar

		# runtime library path
		ldpath+=${ldpath:+:}${oracle_home}/$libdir
