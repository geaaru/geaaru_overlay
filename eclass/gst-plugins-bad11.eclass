...
inherit eutils multilib versionator gst-plugins11
...
# This list is current for gst-plugins-bad-0.11.93
my_gst_plugins_bad="directsound directdraw osx_video quicktime vcd
assrender apexsink bz2 cdaudio celt cog dc1394 directfb dirac dts
faac faad fbdev flite gsm kate ladspa lv2 libmms
...
if version_is_at_least "0.11.93"; then
...
# 6/2/2006 - zaheerm
if [ "${PN}" != "${MY_PN}" ]; then
RDEPEND="=media-libs/gstreamer-0.11*
		=media-libs/gst-plugins-base-0.11*
		>=dev-libs/glib-2.6"
DEPEND="${RDEPEND}
		sys-apps/sed
		virtual/pkgconfig
...
# -bad-0.11.93 uses orc optionally instead of liboil unconditionally.
# While <0.11.93 configure always check for liboil, it is used only by non-split
...
gst-plugins-bad11_src_unpack() {
#	local makefiles

	unpack ${A}
	has src_prepare ${GSTBAD_EXPF} || gst-plugins-bad11_src_prepare
}

gst-plugins-bad11_src_prepare() {
	# Link with the syswide installed gst-libs if needed
	gst-plugins11_find_plugin_dir
...
gst-plugins-bad11_src_compile() {
	has src_configure ${GSTBAD_EXPF} || gst-plugins-bad11_src_configure ${@}

	gst-plugins11_find_plugin_dir
	emake || die "compile failure"
}

gst-plugins-bad11_src_install() {
	gst-plugins11_find_plugin_dir
