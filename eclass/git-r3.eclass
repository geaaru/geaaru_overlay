...
_git-r3_env_setup() {
	...
	if [[ ${EGIT_AUTH_REPOS} ]]; then

		if [ -z "${EGIT_USERNAME}" ] ; then
			die "EGIT_USERNAME not found"
		fi

		if [ -z "${EGIT_PASSWORD}" ] ; then
			die "EGIT_PASSWORD not found"
		fi

		if [ -z "${EGIT_AUTH_PROTOCOL}" ] ; then
			die "EGIT_AUTH_PROTOCOLT not found"
		fi

		echo "${EGIT_AUTH_PROTOCOL}${EGIT_USERNAME}:${EGIT_PASSWORD}@${EGIT_AUTH_REPOS}" > ~/.git-credentials
