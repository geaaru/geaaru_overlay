...
PATCHES=(
	${FILESDIR}/simplestreams-urls-join.patch
)
...
src_prepare() {
	...
	sed -i 's:zfs version:echo 0.8.4:g' configure.ac || die "Can't sed configure.ac for raft"
	eautoreconf
...
src_compile() {
	...
	cd "${GOPATH}/deps/dqlite" || die "Can't cd to dqlite dir"
	emake CFLAGS="-I${GOPATH}/deps/raft/include" \
		LDFLAGS="-L${GOPATH}/deps/raft"
