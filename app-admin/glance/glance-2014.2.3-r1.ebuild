...
python_install() {
	...
	systemd_dounit "${FILESDIR}"/glance-registry.service
	systemd_dounit "${FILESDIR}"/glance-scrubber.service

	# Copy logging property files
	insinto /etc/glance/
	insopts -m 0440 -o root -g glance
	newins "${FILESDIR}/logging_api.conf" logging_api.conf
	newins "${FILESDIR}/logging_registry.conf" logging_registry.conf
	newins "${FILESDIR}/logging_scrubber.conf" logging_scrubber.conf
}
